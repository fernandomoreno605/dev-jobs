...
function validateCreate(request, response, next) {
  const result = validateJob(request.body)
  if (result.success) {
    request.body = result.data // vamos a tener los datos validados y limpios
    return next()
  }

  return response.status(400).json({ error: 'Invalid request', details: result.error.errors })
}

// Middleware de validación para actualización parcial
const validateUpdate = (request, response, next) => {
  const result = validatePartialJob(request.body)
  if (!result.success) {
